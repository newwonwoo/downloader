// Complete-file flow: prepare on the worker, then hand a sized MP4 to the browser.
(() => {
  ...
  function show(job) {
    ...
    if (ready) {
      ...
      document.getElementById('downloadMessage').textContent = `${summary} · 아래 저장 버튼을 누르세요. 임시 파일은 마지막 사용 후 1시간 보관됩니다.`;
    } else if (failed) {
  ...
  async function poll() {
    ...
    try {
      ...
      if ([400, 404, 409, 410, 422, 507].includes(error.status)) {
        ...
        document.getElementById('downloadMessage').textContent = `서버 작업을 다시 조회합니다. ${error.message || ''}`.trim();
        if (failures < 3) timer = setTimeout(poll, 5000);
          ...
          if (!document.getElementById('retryFileJob')) {
            ...
            retry.id = 'retryFileJob';
            retry.textContent = '작업 다시 확인';
      ...
      if (current === generation) polling = false;
    }
  }

  async function start(stream) {
    if (active && !['ready', 'failed'].includes(active.status)) {
      await poll();
      return;
    }
    saveLink().hidden = true;
