...
public final class MainActivity extends Activity {
    ...
    private final Runnable timeoutRunnable = () -> {
        ...
        setStatus("영상 주소를 아직 찾지 못했습니다. 영상 재생을 한 번 눌러 주세요.");
    ...
    private void configureWebView() {
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
                if (request != null && request.getUrl() != null) {
                    captureCandidate(request.getUrl().toString());
                }
                return null;
            }
            ...
            public void onPageFinished(WebView view, String url) {
                ...
                if (looksLikeChallenge(title, url)) {
                    setStatus("보안 확인이 보이면 완료해 주세요. 완료되면 자동으로 계속 찾습니다.");
                } else if (!hasCandidates()) {
                    setStatus("영상 주소를 찾는 중입니다…");
                    playButton.setVisibility(View.VISIBLE);
                }
            }
        ...
        try {
            ...
            // Normal WebView interception and JS observation remain active.
        }
    }

    private void handleIntent(Intent intent) {
        String shared = extractSharedText(intent);
        String source = extractSourceUrl(shared);
        if (source == null) {
    ...
    static String extractSourceUrl(String raw) {
        ...
        if (isAllowedSource(trimmed)) return trimTrailingPunctuation(trimmed);
    ...
    private void captureCandidate(String raw) {
        ...
        synchronized (candidatesLock) {
            if (candidates.size() >= 12 || candidates.contains(accepted)) return;
    ...
    private void injectCaptureBridge() {
        String script = "(function(){"
                + "if(window.__captureV1)return;window.__captureV1=true;"
    ...
    private void triggerPlayback() {
        ...
        webView.evaluateJavascript("(function(){var v=document.querySelector('video');if(v){v.muted=true;var p=v.play();if(p&&p.catch)p.catch(function(){});v.click();}return !!v;})()", null);
        injectCaptureBridge();
    }
