const FRONTEND_VERSION = 'mobile-entry-v2';
...
function extractPageUrl(value){
  ...
  if(!text)return null;
  const candidates=[text,...(text.match(/https:\/\/[^\s<>"']+/gi)||[])];
  for(const candidate of candidates){try{const u=new URL(candidate.replace(/[),.]+$/,''));if(isNjavUrl(u.href))return u.href}catch{}}
  return null;
}
function normalizePageUrl(value){return extractPageUrl(value)}
...
async function analyzeUrl(value,{source='manual'}={}){
  ...
  if(!url){showError(source==='clipboard'?'복사한 내용에 NJAVTV 영상 주소가 없습니다. 영상 페이지에서 공유 → 링크 복사를 한 뒤 다시 눌러주세요.':'NJAVTV 영상 페이지 주소를 확인해 주세요.');return false;}
...
async function analyzeClipboard(){
  if(!window.isSecureContext||!navigator.clipboard?.readText){showError('이 브라우저에서는 자동 붙여넣기를 사용할 수 없습니다. 아래 주소 직접 입력을 눌러 붙여넣어 주세요.');$('manualEntry')?.setAttribute('open','');return;}
  try{
    const text=await navigator.clipboard.readText();
    if(!text.trim())throw new Error('empty');
    await analyzeUrl(text,{source:'clipboard'});
  }catch(error){
    if(error?.message==='empty')showError('복사된 주소가 없습니다. NJAVTV에서 공유 → 링크 복사를 먼저 해주세요.');
    else showError('삼성 인터넷에서 클립보드 읽기를 허용하지 않았습니다. 다시 누르거나 아래 주소 직접 입력을 사용해 주세요.');
  }
}
